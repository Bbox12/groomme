...
<head>
    ...
    <title>EDIT Secondary Service</title>
         ...
         var i;
             if(arr.length!=0){
                               ...
                               for( i=0;i<arr.length;i++){
                                  ...
                                  document.getElementById('Name').value=arr[i].Service;
                                 document.getElementById('PName').value=arr[i].PrimaryService;
                    
                               }
             }
  }
}
 xmlhttp.open("POST","getSecondaryService.php",true);
  ...
  <h3 class="menu-title">Extras</h3><!-- /.menu-title -->
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li class="active">Add Secondary Service</li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="content mt-3">
            <div class="animated fadeIn">
                ...
                <div class="row">
                                              ...
                                              <div class="col-lg-12">
                                                 <div class="card">
                                                    <div class="card-header">
                                                        <strong>Edit Secondary Service</strong>
                                                    </div>
                                                    <div class="card-body card-block">
                                                        <form action="editSecondaryWorkingSite.php" method="post" enctype="multipart/form-data" target="_self" class="form-horizontal">
                                    ...
                                    <div class="col-9 col-md-9"><input type="text" id="ID" name="ID" placeholder="Enter the name" class="input-sm form-control-sm form-control"  readonly></div>
                                                       <div class="col col-md-3"><label for="text-input" class=" input-sm form-control-sm form-control-label">Primary Service*</label></div>
                             <div class="col-8 col-md-8">  <select  id="PName" name="PName" class="input-sm form-control-sm form-control"  required >
   <option value="0">--Select--</option>
        ...
        echo "Could not connect to DBMS"; 
         }else {  $sql =$conn->query("SELECT `ID`, `Name` FROM `primary_services` ");
        foreach($sql as $row) {
    ?>
...
?>
                             ...
                             <div class="col col-md-3"><label for="text-input" class=" input-sm form-control-sm form-control-label">Secondary Service&nbspName</label></div>
                      ...
                      $conn = $db->connect();
                       ...
                       $users =$conn->query("SELECT `ID`, `Service`, (SELECT Name FROM primary_services WHERE ID=s.IDPrimaryService) AS primaryservice,`Photo`, `Ladies`, `Gents`, `Bridal`, `Tattoo`, `Kids` FROM `secondary_service`  s WHERE isActive=1");
                        if(!empty($users)): $count = 0; foreach($users as $user): $count++;
                    ?>
                    <tr>
                        <td><?php echo $count; ?></td>
                          <td><a href="EditSecondaryService.php?id=<?php echo $user['ID']; ?>" style="display:block;color: blue;"><u><?php echo $user['Service']; ?></u></a></td>
                                     <td><?php echo $user['primaryservice']; ?></td>
                          <td><img src="<?php echo 'http://' . $server_ip .'/' .'Groom' .'/'.'Dashboard'.'/'.'service'.'/'.$user["Photo"];?>" width='48' height='48'></td>
                    ...
                    <?php endif; ?>
                        ...
                        <div class="col-lg-6" >
                            ...
                            <div class="card-header">
                                <strong class="card-title">In Active Secondary services</strong>
                            </div>
                      ...
                      $conn = $db->connect();
                        $date=date("Y-m-d");
                         $server_ip="139.59.38.160";
                        $users =$conn->query("SELECT `ID`, `Service`, (SELECT Name FROM primary_services WHERE ID=s.IDPrimaryService) AS pro,`Photo`, `Ladies`, `Gents`, `Bridal`, `Tattoo`, `Kids` FROM `secondary_service`  s WHERE isActive=0");
                        if(!empty($users)): $count = 0; foreach($users as $user): $count++;
                    ?>
                    <tr>
                        <td><?php echo $count; ?></td>
                        <td><?php echo $user['Service']; ?></td>
                                     <td><?php echo $user['pro']; ?></td>
                          <td><img src="<?php echo 'http://' . $server_ip .'/' .'Groom' .'/'.'Dashboard'.'/'.'service'.'/'.$user["Photo"];?>" width='48' height='48'></td>
